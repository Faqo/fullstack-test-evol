...
describe('TasksService', () => {
  ...
  it('should create and find tasks', async () => {
    ...
    // Test create
    mockTaskModel.create.mockResolvedValue(mockTask as Task);
    const created = await service.create({ title: 'Test' });
    expect(created).toEqual(mockTask);

    // Test findAll
    mockTaskModel.findAll.mockResolvedValue([mockTask] as Task[]);
  ...
  it('should update and get tags', async () => {
    ...
    mockTaskModel.findByPk.mockResolvedValue(mockTask as unknown as Task);
